...
Parallelism and resumability:
  --n-jobs N       run permutations across N worker processes (linear speedup).
  --checkpoint F   append one row per completed permutation (crash-safe resume).
  --max-hours H    stop gracefully after H hours of wall time (exit 0).
  --resume F       skip permutations already present in checkpoint F.
Permutation randomness depends only on random_seed + perm_index, so a resumed
run reproduces the same null sequence as an uninterrupted run.
"""
...
def run_permutation_null(
    ...
    *,
    ...
    """Run the permutation null, optionally in parallel with checkpointing."""
...
def main(config_path: Path, n_permutations: int, n_jobs: int, checkpoint: Path | None, max_hours: float | None) -> None:
    ...
    print("running observed nested OOF (serial)...", flush=True)
